...
import com.thoughtworks.qdox.type.*;
...
public class JavaClassUtil {
    ...
    private static List<AnnotationValue> getAnnotationValues(List<String> validates, JavaAnnotation javaAnnotation) {
        ...
        if (simpleName.equalsIgnoreCase(ValidatorAnnotations.VALIDATED) || simpleName.equalsIgnoreCase(ValidatorAnnotations.VALID)) {
            if (Objects.nonNull(javaAnnotation.getProperty(DocAnnotationConstants.VALUE_PROP))) {
                ...
                try {
                    annotationValueList = new ArrayList<>();
                    DefaultJavaParameterizedType type = (DefaultJavaParameterizedType) javaAnnotation.getType();
                    TypeResolver typeResolver = TypeResolver.byPackageName(type.getSource().getPackageName(),
                            type.getSource().getJavaClassLibrary(),
                            type.getSource().getImports());
                    annotationValueList.add(new TypeRef(new DefaultJavaParameterizedType("javax.validation.groups.Default", "Default", 0, typeResolver)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
